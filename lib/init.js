...
module.exports = function (app, options) {
    ...
    passport.use(new Strategy(strategy.config, strategy.verify(
      function (req, profileId, profile, done) {
        ...
        model.fetch(query, function (err) {
          ...
          model.set('_session.' + options.session.path + '.id', userId);
          model.set('_session.' + options.session.path + '.registered', true);
          dotty.put(req.session, options.session.path + '.id', userId);
          dotty.put(req.session, options.session.path + '.registered', true);
          done(null, {id: userId});
        });
      }
    )));
  ...
  return function (req, res, next) {
    ...
    var saveSession = function () {
      dotty.put(req.session, options.session.path + '.id', userId);
      dotty.put(req.session, options.session.path + '.registered', isRegistered);
      model.set('_session.' + options.session.path + '.registered', isRegistered);
      model.set('_session.' + options.session.path + '.id', userId);
    };
