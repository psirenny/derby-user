...
module.exports = function (options) {
  ...
  app.post('/changeEmail', function (req, res) {
    ...
    model.fetch($user, $find, function (err) {
      ...
      if (found && found.id === userId) return res.json(null);
      if (found && found.id !== userId) return res.status(400).json({error: 'email in use'});
      var token = model.id();
  ...
  app.post('/changePassword', function (req, res, next) {
    ...
    $user.fetch(function (err) {
      ...
      credential.verify(hashedPassword, currentPassword, function (err, valid) {
        ...
        if (!valid) return next(null, false, {error: 'invalid current password'});
  ...
  app.get('/confirmEmail/:id/:token', function (req, res, next) {
    ...
    $user.fetch(function (err) {
      ...
      var hashedToken = $user.get('local.emails.0.token.hash');
      if (!hashedToken) return res.redirect('../../confirmedEmail');
      credential.verify(hashedToken, token, function (err, valid) {
        if (err) return next(err);
  ...
  app.get('/confirmedEmail', function (req, res) {
    // override "/confirmedEmail" to change this redirect
    res.redirect('/');
  });
  ...
  app.post('/resetPassword', function (req, res, next) {
    ...
    $user.fetch(function (err) {
      ...
      if (!hashedToken) return res.status(400).json({error: 'invalid token'});
      credential.verify(hashedToken, token, function (err, valid) {
        if (err) return res.status(500).json({error: err});
  ...
  app.get('/signout', function (req, res, next) {
    req.duser.signout(next);
  });

  app.post('/signout', function (req, res) {
    req.duser.signout(function (err, obj) {
